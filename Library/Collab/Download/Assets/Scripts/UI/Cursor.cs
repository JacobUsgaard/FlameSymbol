...
public class Cursor : FocusableObject
{
    ...
    public void CharacterActionMenuOnCancel(Object[] objects)
    {
        ...
        SelectedCharacter.CalculateAttackableSpaces();
    ...
    private void CharacterActionMenuAttack(Object[] objects)
    {
        ...
        foreach (Item item in SelectedCharacter.GetAttackableItems())
        {
    ...
    private void ItemActionMenuEquip(Object[] objects)
    {
        ...
        List<Item> items = SelectedCharacter.Items;
        if(items.Count <= 1)
        {
            return;
        }
    ...
    {
        Move(endposition, CurrentState);
    }

    private void Move(Vector3 endPosition, State nextState)
    {
    ...
    public void SetTradableSpaceWithCharacter(int index = 0)
    {
        ...
        //GameManager.AttackDetailPanel.Show(GameManager.GetCharacter(CurrentSelectedPosition), GameManager.GetCharacter(transform.position));
    }
    ...
    private void OnSubmitChoosingAttackTarget()
    {
        ...
        attacker.DestroyAttackableSpaces();
    ...
    private void OnSubmitChoosingMove(Vector3 currentPosition)
    {
        ...
        foreach (Transform movableSpace in SelectedCharacter.CalculateMovableSpaces())
        {
            if (currentPosition.x == movableSpace.position.x && currentPosition.y == movableSpace.position.y)
            {
                ...
                SelectedCharacterOldPosition = SelectedCharacter.CalculatePosition();
    ...
    public override void OnCancel()
    {
        switch (CurrentState)
        {
            ...
            case State.ChoosingMove: // --> Free
                ...
                SelectedCharacter.DestroyAttackableSpaces();
                SelectedCharacter.DestroyMovableSpaces();
                CurrentState = State.Free;
                break;
